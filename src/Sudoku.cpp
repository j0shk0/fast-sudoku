...
void Sudoku::generate() {
  ...
  for (int i = 0; i < 9; i++) {
    for (int j = 0; j < 9; j++) {
      ...
      boxState_[(((3 * box_row) + box_col) * 9) + (board_[(i * 9) + j] - 1)] =
          true;
    }
  }
}

Sudoku::Sudoku(int solution[81]) { std::copy_n(solution, 81, board_); }
...
bool Sudoku::checkColumn(const size_t& row, const size_t& col) const {
  ...
  const bool existing_value =
      columnState_[(col * 9) + (board_[(row * 9) + col] - 1)];
  return !existing_value;
}
...
bool Sudoku::checkBox(const size_t& row, const size_t& col) const {
  ...
  const bool existing_value = boxState_[(((3 * box_row) + box_col) * 9) +
                                        (board_[(row * 9) + col] - 1)];
  return !existing_value;
}
...
bool Sudoku::check() {
  for (auto i = 0; i < 9; i++) {
    for (auto j = 0; j < 9; j++) {
      if (board_[(i * 9) + j] < 1 || board_[(i * 9) + j] > 9) {
        return false;
      }
      [[unlikely]] if (!checkRow(i, j) || !checkColumn(i, j) ||
                       !checkBox(i, j)) {
        return false;
      }
      rowState_[board_[(i * 9) + j] - 1] = true;
...
std::string Sudoku::getBoardString() const {
  std::string result;
  result.reserve(81);
  for (const int i : board_) {
